...
#' @param shape shape of the points to be plotted (only used when not plotting by group) 
...
#' @examples
#'
#' Thie example will plot the points on the secondary y-axis with error bars and labels showing 
#' the plotted values that are vertically adjusted from the plotted points
#' geom_chart(df = plot_df, x = "age", y = "value", ci = "e", upper = "uppercl", lower = "lowercl", 
#' group_var = "ukborn", y_axis = "y2", y_label = "Value", labels = "value", vjust = -3.5)
#'
#'
...
no_shift = FALSE) {
  ...
  ifelse(!missing(group_var),
         ...
         base <- base + ggplot2::geom_point(data = df, aes(x = .data[[x]], y = .data[[y]],group = .data[[group_var]], colour = .data[[group_var]], shape = .data[[group_var]])),

         base <- base + ggplot2::geom_point(data = df, aes(x = .data[[x]], y = .data[[y]]),color = "blue", shape = shape)
  ...
  if(!(missing(ci)) && missing(group_var)){
    ...
    ifelse(!(missing(lower)) && !(missing(upper)),
           ...
           ifelse(ci == 'e',
                  ...
                  base <- base + ggplot2::geom_errorbar(data = df, aes(x = .data[[x]], ymin = .data[[lower]], ymax = .data[[upper]]), colour = error_colour, linewidth = 0.5)
  ...
  if(!(missing(ci)) && !missing(group_var)){
    ...
    ifelse(!(missing(lower)) && !(missing(upper)),
           ...
           ifelse(ci == 'e',
                  ...
                  # Apply error bar with separate legends for line and ci
                  base <- base + ggplot2::geom_errorbar(data = df, aes(x = .data[[x]], ymin = .data[[lower]], ymax = .data[[upper]],group = .data[[group_var]], colour =  .data[[group_var]]), linewidth = .5)
                  ,
  ...
  if(!missing(labels)){
    base <- base + geom_text(data = df, aes(x = .data[[x]], y = .data[[y]], label = round(.data$original_label,0)),hjust=hjust, vjust=vjust)
  }
