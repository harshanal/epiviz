...
#' epi_map
#'
#' The map function has 2 mandatory arguments to plot, df (data frame containing your data) and value_col (value variable name),
#' with additional arguments as described further below:
#'
#' @param df name of data frame for plotting
#' @param value_col name of value variable in data frame
#' @param inc_shp boolean parameter to indicate whether df includes shapefile data or if this will be uploaded separately.
#' @param shp_filepath filepath for the shapefile required for spatial data in function. Note this will not be uploaded if inc_shp = TRUE.
#' @param shp_areacode name of areacode variable in shapefile (Mandatory if shp_filepath argument passed).
#' @param data_areacode name of areacode variable in dataframe (Mandatory if shp_filepath argument passed).
#' @param lookup vector of categories to include in the legend.
#' @param break_intervals numeric vector of interval points for the legend (Mandatory if lookup argument passed).
#' @param force_cat boolean parameter to determine whether all arguments passed in the lookup argument are used, even if there are no values present in the data.
#' @param n_breaks number of break intervals. This argument is an alternative to supplying defined breaks in lookup argument and will
#' provide a number of evenly distributed breaks as specified. Note if lookup argument is passed this will be ignored.
#' @param legend_title string containing the legend title.
#' @param legend_pos string containing legend position.
#' @param border_shape_filepath filepath for the shapefile if an outer border is required for the map. This should be a higher geography e.g.
#' if creating an UTLA map- a regional shapefile can be used.
#' @param border_code_col name of areacode variable in the border shapefile. Required if a specific area within the border shapefile is required.
#' @param border_areaname string containing the name of area required for border shapefile (required if border_code_col argument passed).
#'
#' @import classInt
#' @import RColorBrewer
#' @export
...
#' various visualisations, using different arguments.
#'
#' Example 1: Displays a map for rates per 100,000 across UTLA geographies in London with a border for the outer London region boundaries.
#' The data frame contains shapefile data which isn't imported separately.  The legend contains specified legend categories with all levels
#' forcefully included, with a legend title stating "Rate per 100,000" and positioned on the left of the map.
#' --------------------------------------------------------------------------------------------------------
#' leg_cats <- c("0.0-0.9 (pre-elimination)", "1.0-4.9 (low incidence)", "5.0-9.9 (low incidence)",
#'             "10.0-14.9", "15.0-19.9", "20-29.9", "30.0-39.9", ">=40.0")
#'
...
#'              legend_pos = "left")
#'
#' Example 2: Displays a map for rates per 100,000 across UTLA geographies in London without a border.
#' The data frame contains shapefile data which isn't imported separately.  The legend includes 5 breaks which are
#' not pre-defined, does not include a legend title and is positioned to the right of the map.
#' --------------------------------------------------------------------------------------------------------
#' epi_map(df = map,
#'              inc_shp= TRUE,
#'              value_col = "value",
...
epi_map <- function(df,
  ...
  if (missing(value_col)) stop("Please inlcude argument data frame variable for value, ie value = variable_name")
  ...
  if(inc_shp == TRUE ) {
    ...
    data_sf_merged <- merge(area_sf, df, by.x = shp_areacode, by.y = data_areacode)
  ...
  if(is.null(lookup)) {
    ...
    breaks_qt <- classIntervals(c(min(unlist(data_sf_merged$Value)) - .00001, unlist(data_sf_merged$Value)), n = n_breaks
                                , style = "quantile")
    ...
    breaks_qt <- data.frame(interval = interval_grp , value_cat = lookup)
    ...
    order <- data.frame(interval= data_sf$interval, value_cat = data_sf$value_cat) %>%
      arrange(interval) %>%
      distinct()

    # Re-order factor to correct order
    data_sf <- data_sf %>%
      mutate(value_cat = factor(value_cat, order$value_cat),
             interval = as.factor(interval))
  ...
  if(!is.null(border_shape_filepath)) {
    ...
    if(!is.null(border_code_col)) {
      ...
      index <- as.numeric(grep(border_code_col, colnames(border_sf)))
