...
#' @param line_colour colour (or vector of colours if group_var != NULL) for
#' the line in the chart
#' @param error_colour if ci != NULL - colour (or vector of colours if group_var
#'  != NULL) for the error bar or ribbon in the chart
#' @param ci "e" for error bars or "ribbon" for a ribbon - lower and upper become
...
#' @param y_axis y axis to use - y1 or y2 (default is y1 - standard left-hand axis,
#'  y2 can be used for dual-axis graphs)
#' @param h_line will display a horizontal line if valid inter passed
#' @param line_type if provided will change all lines used to that type i.e.
...
#' @param line_size will alter line width accordingly for all lines generated,
#' defaults to 1
#' @param x_label for provision of an x axis label
#' @param y_label for provision of an y axis label
#' @param x_label_angle to adjust the x axis label by the degrees of the integer
#' provided
#' @param y_label_angle to adjust the y axis label by the degrees of the integer
#' provided
...
#' top, right, left
#' @param remove_gridlines enter an argument of any value i.e. 'y' to remove the
#' grid lines
...
line_chart <- function(df,
  ...
  if (missing(x))
    stop("Please include argument data frame variable for x axis,
  ...
  # Determine breaks for either discrete or contentious x variable using Scales
  # package function
